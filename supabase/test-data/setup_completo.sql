-- ============================================
-- CONFIGURACIÓN COMPLETA EN UN SOLO SCRIPT
-- ============================================
-- Este script configura TODO automáticamente:
-- 1. Crea las tablas de administración de alquileres
-- 2. Crea una orden del servicio de administración
-- 3. Crea datos de prueba (propiedad, inquilino, pagos, gastos, incidencias)
--
-- IMPORTANTE: Reemplaza 'TU_EMAIL_AQUI' con tu email de usuario
-- ============================================

DO $$
DECLARE
  v_user_id uuid;
  v_user_email text := 'TU_EMAIL_AQUI'; -- ⬅️ CAMBIA ESTO POR TU EMAIL
  v_service_id uuid;
  v_order_id uuid;
  v_property_id uuid;
  v_tenant_id uuid;
BEGIN
  RAISE NOTICE '=== INICIO DE CONFIGURACIÓN ===';
  RAISE NOTICE '';
  ...
  -- Paso 2: Buscar el servicio de administración de alquiler
  SELECT id INTO v_service_id FROM public.services WHERE slug = 'administracion-alquiler' LIMIT 1;
  
  IF v_service_id IS NULL THEN
    RAISE EXCEPTION '❌ Servicio "administracion-alquiler" no encontrado. Ejecuta primero la migración 20260511000000_add_administracion_alquiler_service.sql';
  END IF;
  
  RAISE NOTICE '✓ Servicio de administración encontrado';

  -- Paso 3: Crear orden (si no existe ya)
  ...
  IF v_order_id IS NULL THEN
    ...
    RAISE NOTICE '✓ Orden de suscripción creada: %', v_order_id;
  ELSE
    RAISE NOTICE '✓ Ya tienes una orden de administración: %', v_order_id;
  END IF;
  ...
  -- Paso 6: Crear pagos de alquiler
  ...
  RAISE NOTICE '✓ 5 pagos de alquiler creados';

  -- Paso 7: Crear gastos del inmueble
  ...
  RAISE NOTICE '✓ 6 gastos del inmueble creados';

  -- Paso 8: Crear incidencias
  ...
  RAISE NOTICE '✓ 3 incidencias creadas';
  ...
  RAISE NOTICE '   • 1 propiedad (Gran Vía 45, Madrid)';
  RAISE NOTICE '   • 1 inquilina (María García López)';
  RAISE NOTICE '   • 5 pagos (3 pagados, 2 pendientes)';
  RAISE NOTICE '   • 6 gastos del inmueble';
  RAISE NOTICE '   • 3 incidencias';
