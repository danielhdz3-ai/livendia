...
import { CONTRATO_ARRAS_ONLINE_GESTOR } from "@/lib/contrato-arras-local-lead-content";
import { CheckCircle, FileSearch, Headphones, Laptop, User } from "lucide-react";
...
export function ContratoArrasOnlineGestorSection({ city, legalRegion = "catalunya" }: Props) {
  const copy = CONTRATO_ARRAS_ONLINE_GESTOR;
  const place = city ? ` en ${city}` : "";
  const gestorSteps =
    legalRegion === "espana" ? copy.gestorStepsEspana : copy.gestorStepsCatalunya;
        ...
        <div className="mt-12 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:gap-6">
            ...
            <div className="flex-1">
              <h3 className="text-xl font-bold text-[#1E293B]">{copy.postArrasTitle}</h3>
              <p className="mt-3 leading-relaxed text-[#64748b]">{copy.postArrasBody}</p>
              <div className="mt-5 flex flex-wrap gap-3">
                <Link
                  href={copy.postArrasLinkHref}
                ...
                >
                  ...
                  {copy.postArrasLinkLabel}
                </Link>
                <Link
                  href={copy.ventaLinkHref}
                  className="inline-flex items-center gap-2 rounded-full border border-slate-300 px-5 py-2.5 text-sm font-semibold text-[#475569] hover:bg-slate-50"
                >
                  {copy.ventaLinkLabel}
                </Link>
              </div>
