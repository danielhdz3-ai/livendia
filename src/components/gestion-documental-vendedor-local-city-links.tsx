...
export function GestionDocumentalVendedorLocalCityLinks({
  showTitle = true,
  variant = "default",
}: Props) {
  const cities = getPublishedGestionDocumentalVendedorLocalCities();
  ...
  return (
    <div className={isCompact ? "min-w-0 space-y-1.5" : "space-y-3"}>
      {showTitle ? (
        ...
        >
          {isFooter
            ? "Gestión documental vendedor por ciudad"
            : "Gestión documental vendedor post-arras por ciudad"}
        </p>
      ) : null}
      <nav
        aria-label="Enlaces a gestión documental vendedor por ciudad en España"
        className={wrapClass}
      >
        {cities.map((c) => (
          <Link key={c.slug} href={localGestionDocumentalVendedorHref(c.slug)} className={linkClass}>
            {c.city}
          </Link>
        ))}
      </nav>
      <Link
        href={GESTION_DOCUMENTAL_VENDEDOR_LOCAL_BASE}
        className={
          isCompact
      ...
      >
        {isFooter ? "Servicio nacional →" : "Ver gestor documental vendedor (España) →"}
      </Link>
    </div>
  );
}
