...
export function ServicioCompletoCompraLocalCityLinks({
  showTitle = true,
  variant = "default",
}: Props) {
  const cities = getPublishedServicioCompletoCompraLocalCities();
  ...
  return (
    <div className={isCompact ? "min-w-0 space-y-1.5" : "space-y-3"}>
      {showTitle ? (
        ...
        >
          {isFooter ? "Compra completa por ciudad" : "Servicio completo de compra por ciudad"}
        </p>
      ) : null}
      <nav
        aria-label="Enlaces a servicio completo de compra por ciudad en España"
        className={wrapClass}
      >
        {cities.map((c) => (
          <Link key={c.slug} href={localServicioCompletoCompraHref(c.slug)} className={linkClass}>
            {c.city}
          </Link>
        ))}
      </nav>
      <Link
        href={SERVICIO_COMPLETO_COMPRA_LOCAL_BASE}
        className={
          isCompact
