...
export function RevisionDocumentalPostArrasLocalCityLinks({
  showTitle = true,
  variant = "default",
}: Props) {
  const cities = getPublishedRevisionDocumentalPostArrasLocalCities();
  ...
  return (
    <div className={isCompact ? "min-w-0 space-y-1.5" : "space-y-3"}>
      ...
      <nav
        aria-label="Enlaces a revisión documental post-arras por ciudad en España"
        className={wrapClass}
      >
        {cities.map((c) => (
          <Link
            key={c.slug}
            href={localRevisionDocumentalPostArrasHref(c.slug)}
            className={linkClass}
          >
            {c.city}
          </Link>
      ...
      <Link
        href={REVISION_DOCUMENTAL_POST_ARRAS_LOCAL_BASE}
        className={
          isCompact
      ...
      >
        {isFooter ? "Pack nacional →" : "Ver pack post-arras (España) →"}
      </Link>
    </div>
  );
}
