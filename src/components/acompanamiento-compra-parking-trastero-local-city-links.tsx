...
export function AcompanamientoCompraParkingTrasteroLocalCityLinks({
  showTitle = true,
  variant = "default",
}: Props) {
  const cities = getPublishedParkingTrasteroLocalCities();
  ...
  return (
    <div className={isCompact ? "min-w-0 space-y-1.5" : "space-y-3"}>
      {showTitle ? (
        ...
        >
          {isFooter
            ? "Compra parking/trastero por ciudad"
            : "Acompañamiento compra parking y trastero por ciudad y barrio"}
        </p>
      ) : null}
      <nav aria-label="Enlaces a compra parking y trastero por ciudad en España" className={wrapClass}>
        {cities.map((c) => (
          <Link key={c.slug} href={localAcompanamientoCompraParkingTrasteroHref(c.slug)} className={linkClass}>
            {c.city}
          </Link>
        ))}
      </nav>
      ...
      >
        {isFooter ? "Índice local →" : "Ver todas las ciudades y barrios →"}
      </Link>
    </div>
  );
}
