...
export function ContratoAlquilerOpcionCompraLocalCityLinks({
  showTitle = true,
  variant = "default",
}: Props) {
  const cities = getPublishedContratoAlquilerOpcionCompraLocalCities();
  ...
  return (
    <div className={isCompact ? "min-w-0 space-y-1.5" : "space-y-3"}>
      {showTitle ? (
        ...
        >
          {isFooter ? "Opción a compra por ciudad" : "Alquiler con opción a compra por ciudad"}
        </p>
      ) : null}
      <nav aria-label="Enlaces a alquiler con opción a compra por ciudad" className={wrapClass}>
        {cities.map((c) => (
          <Link key={c.slug} href={localContratoAlquilerOpcionCompraHref(c.slug)} className={linkClass}>
            {c.city}
          </Link>
        ))}
      </nav>
      <Link
        href={CONTRATO_ALQUILER_OPCION_COMPRA_LOCAL_BASE}
        className={
          isCompact
            ? "text-[11px] font-semibold text-cyan-300 hover:text-white"
            : "text-sm font-semibold text-[#1A4FBF] hover:underline"
        }
      >
