...
import { SERVICIO_COMPLETO_VENTA_LOCAL_BASE } from "@/lib/servicio-completo-venta-local-cities";
import { getVentaParticularCityLinks } from "@/lib/vender-piso-sin-agencia-local-cities";
...
export function VentaParticularesLocalCityLinks({
  showTitle = true,
  variant = "default",
}: Props) {
  const cities = getVentaParticularCityLinks();
  ...
  return (
    <div className={isCompact ? "min-w-0 space-y-1.5" : "space-y-3"}>
      {showTitle ? (
        ...
        >
          {isFooter
            ? "Venta particulares por ciudad"
            : "Acompañamiento de venta particulares por ciudad"}
        </p>
      ) : null}
      <nav aria-label="Enlaces a venta entre particulares por ciudad" className={wrapClass}>
        {cities.map((c) => (
          <Link key={c.slug} href={c.href} className={linkClass}>
            {c.city}
          </Link>
        ))}
      </nav>
      <Link
        href={SERVICIO_COMPLETO_VENTA_LOCAL_BASE}
        className={
          isCompact
