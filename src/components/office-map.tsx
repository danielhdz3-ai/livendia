...
type OfficeMapProps = {
  ...
  title?: string;
  /** Mostrar enlace flotante con la dirección (por defecto sí). */
  showCaption?: boolean;
};
    ...
    <div className={`relative overflow-hidden bg-[#0f274f] ${minHeightClassName} ${className}`}>
      ...
      {showCaption ? (
        <a
          ...
          className="absolute bottom-3 left-3 z-10 max-w-[calc(100%-1.5rem)] truncate rounded-lg bg-white/95 px-3 py-2 text-xs font-semibold text-[#1A4FBF] shadow-md backdrop-blur-sm transition hover:bg-white sm:right-auto sm:max-w-[min(100%,28rem)]"
        >
