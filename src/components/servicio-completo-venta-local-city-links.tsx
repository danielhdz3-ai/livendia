...
export function ServicioCompletoVentaLocalCityLinks({
  showTitle = true,
  variant = "default",
}: Props) {
  const cities = getPublishedServicioCompletoVentaLocalCities();
  ...
  return (
    <div className={isCompact ? "min-w-0 space-y-1.5" : "space-y-3"}>
      {showTitle ? (
        ...
        >
          {isFooter ? "Venta sin agencia por ciudad" : "Acompañamiento de venta por ciudad"}
        </p>
      ) : null}
      <nav aria-label="Enlaces a venta con gestor por ciudad" className={wrapClass}>
        {cities.map((c) => (
          <Link key={c.slug} href={localServicioCompletoVentaHref(c.slug)} className={linkClass}>
            {c.city}
          </Link>
        ))}
      </nav>
