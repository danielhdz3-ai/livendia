...
export type RentalPropertyDocRow = {
  id: string;
...
export function RentalPropertyDocuments({
  propertyId,
  initialDocs,
}: {
  propertyId: string;
  initialDocs: RentalPropertyDocRow[];
}) {
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/rental/property-document", { method: "POST", body: fd });
      const data = (await res.json()) as {
        error?: string;
        document?: RentalPropertyDocRow;
      };
      ...
      if (data.document) {
        setDocs((d) => [data.document as RentalPropertyDocRow, ...d]);
      }
      router.refresh();
    } catch {
      setErr("Error de red");
  ...
  async function downloadRow(d: RentalPropertyDocRow) {
  ...
  async function removeDoc(d: RentalPropertyDocRow) {
    if (!globalThis.confirm("¿Eliminar este archivo del inmueble?")) return;
    setBusy(true);
    setErr(null);
    try {
      const res = await fetch(`/api/rental/property-document?id=${encodeURIComponent(d.id)}`, {
        method: "DELETE",
      });
  ...
  return (
    <div>
      <p className="text-sm text-[#64748B]">
        Sube o consulta escrituras, IBI, cédulas y otros documentos del inmueble. Solo tú tienes acceso.
      </p>
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-end">
        <div className="min-w-[160px] flex-1">
          <label htmlFor={`prop-doc-type-${propertyId}`} className="text-xs font-medium text-[#64748b]">
            Tipo de documento
          </label>
          <select
            id={`prop-doc-type-${propertyId}`}
            value={docType}
      ...
      {!docs.length ? (
        <p className="mt-6 text-center text-sm text-[#64748B]">
          Todavía no hay archivos asociados. Puedes añadirlos con el botón anterior.
        </p>
      ) : (
